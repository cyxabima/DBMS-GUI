...
def add_record(metadata, data):
    ...
    name and field length from metadata and uses these to add records in a data"""

    while True:
        new_record = {}
        print("*" * 100)
        for field in metadata:
            # metadata is a list of dict so field  containe dictionery
            field_name = field["field name"]
            field_length = field["field length"]

            # here we are using while True because we want user to keep on entring the value unless
            # it is a correct value meaning it should not be greater than provided length
            while True:
                field_value = input(f"'{field_name}' : ")
                if len(field_value) > field_length:
                    print(
                        f"Value exceeds the maximum length of {field_length}\
                            characters. Please enter a shorter value."
                    )
                    # continue
                else:
                    # new record is a dictionery which was initilized when outer while loop start)
                    new_record[field_name] = field_value
                    break

        data.append(
            new_record
        )  # data is a list and dict named new_record is appanended to the list
...
def edit_record(metadata, data):
    ...
    if not records:
        ...
        print("=" * 70)
        record_index = input(
            "Enter the record number to edit (or press enter to cancel): "
        )
        if record_index.isdigit():
            ...
            if 0 <= record_index < len(records):
                ...
                for field in metadata:
                    ...
                    while True:
                        new_value = input(
                            f"Current '{field_name}': {current_value}. Enter new value (max length {
                                field_length}): "
                        )
                        if new_value and len(new_value) > field_length:
                            print(
                                f"Value exceeds the maximum length of {field_length} characters.\
                ...
                edit_record(metadata, data)  # recursive call
        elif not record_index:
            print("You cancel the Edit")
            print("=" * 70)
        else:
            print("Plz Enter Valid Inetger")
            edit_record(metadata, data)  # recursive call


def del_record(metadata, data):
    """Display All record and ask user to enter record no which he want to delete
    and pop that record from list if record no doesnot exist show message that Invalid record
    """
...
def view_record(metadata, data):
    """Take data and metadata as input use metadata to extract
    field and lenght by using these two arguments create a list
    of string with padding(get from length) in it then insert
     another element sno in the list and then use .join method to make a
     string separated by | (pipe) and print that header"""
    ...
    list_headerContent = [
        ...
        # using list comprehension to make a list of strings with padding(field and length)
    ]
    list_headerContent.insert(
        0, f'{"S.No":<7}'
    )  # adding s.no with padding of 7 to list 0th index
    ...
    for record in data:  # here data is list of dict so record contain a dict
        count += 1
        list_rowContent = [
            f"{record[header]:<{length}}" for header, length in zip(headers, lengths)
        ]
...
def search_record(metadata, data):
    ...
    for index, dic in enumerate(metadata, start=1):
        # making a dictionery with key is the serial no of fields in integer form so
        # that user can type only no of field not the field name
        dic_choice.update({index: f"{dic['field name']}"})
        print(f"{index} : {dic['field name']}")

    choice = input("Enter you choice: ")

    # here i am first checking that choice is digit or not if it is digt convert it into interger now check
    # weather that integer is less than or eqal to len of dic_choice(which is a dict containing key as number and
    # value as name of field extracted from metadata). then we are looping over that dictionery if user choice matches
    # with the key in that field. ask user to enter the value he wnat to find in that field which he selected through
    # his choice.
    ...
    if choice.isdigit():
        ...
        if 0 < choice <= len(dic_choice):
            ...
            for keys, values in dic_choice.items():
                ...
                if choice == keys:
                    finding = input(
                        f"Enter The value you want to find in field of {
                            values}: "
                    )
                    # inintially no record found therefore Flase but if found make it True
                    # otherwise it will remains False
                    is_record_found = False
                    serial_no = 0  # for serial no
                    for dic in records:  # dic conatine the dict of each record

                        serial_no += (
                            1  # incrementing the value of serial no for each iteration
                        )

                        if dic[values] == finding:
                            # here value contain that field name which is chosen by user so for every dic in records f
                            print("=" * 70)
