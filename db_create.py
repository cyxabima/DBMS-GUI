...
def create_db():
    ...
    if database_name in databases:
        ...
        print("-" * 80)
        create_db()  # again calling the same function
    else:
        # database = {}
        metadata = []
        while True:
            ...
            if field_name == "":
                break
            while True:
                # we using this while loop to take input from user again if the length enter by use is not integer
                field_length = input("Enter length for this field (integer): ")
                if field_length.isdigit():
                    metadata.append(
                        {"field name": field_name, "field length": int(field_length)}
                    )
                    break
